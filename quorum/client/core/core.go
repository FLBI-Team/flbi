...
func (b *Benchmark) measure(
	...
	lastH, err := b.client.HeaderByNumber(context.Background(), nil)
	if err != nil {
		panic(err)
	}
	...
	curH, _ := b.client.HeaderByNumber(context.Background(), nil)
	for n := lastH.Number; curH.Number.Cmp(n) == 1; n = n.Add(n, big.NewInt(1)) {
		header, _ := b.client.HeaderByNumber(context.Background(), n)
